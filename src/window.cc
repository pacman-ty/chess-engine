...
Xwindow::Xwindow(int width, int height) : width{width}, height{height} {
    ...
    XColor xcolour;
    Colormap cmap;
    char color_vals[5][10]={"white", "black", "red", "gray", "blue"};
...
void Xwindow::drawString(int x, int y, string msg, int colour) {
    ...
    int count = 0;
    char** lst =  XListFonts(d, "-*-*-*-*-*-*-*-*-*-*-*-*-adobe-*", 10, &count);
    XFontStruct *fontInfo = XLoadQueryFont(d, "-bitstream-bitstream charter-bold-i-normal--17-120-100-100-p-0-adobe-standard");
    if (!fontInfo) {
        exit(-1);
    }
